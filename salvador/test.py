...
def main():
    ...
    helper = argparse.ArgumentParser(prog="test_vega", description="The Salvador Testing Application using randomly generated, large sparse matrices.")
    ...
    for i in range(num_tests):
        ...
        logger.info("Salvador Algorithm with an approximate solution started")
        started = time.time()
        
        novel_result = algorithm.find_vertex_cover(graph)

        logger.info(f"Salvador Algorithm with an approximate solution done in: {(time.time() - started) * 1000.0} milliseconds")

        answer = utils.string_result_format(novel_result, count)
        output = f"{i + 1}-Salvador Test: {answer}" 
        utils.println(output, logger, args.log)

        if novel_result and (bruteForce or approximation):
            if bruteForce:    
                output = f"Exact Ratio (Salvador/Optimal): {len(novel_result)/len(brute_force_result)}"
            elif approximation:
                output = f"Upper Bound for Ratio (Salvador/Optimal): {2 * len(novel_result)/len(approximate_result)}"
            utils.println(output, logger, args.log)
